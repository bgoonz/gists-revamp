export const useCreatePost = ({topicSlug, pageNumber, pageSize }) => useMutation(CREATE_POST, {
  update(cache, {data:{createPost}}) {
    try {
      ...
      const actualPage = postsByTopic.posts.length >= pageSize ? pageNumber + 1 : pageNumber
      ...
      Object.keys(cache.data.data.ROOT_QUERY).forEach(key => {
        if (key.match(/^postsByTopic/)) {
          ...
          const queryFieldsObj = JSON.parse(key.substring("postsByTopic(".length, key.length -1))
          cache.writeQuery({
            query:GET_POSTS_BY_TOPIC,
            data:{
              postsByTopic: {
            ...
            variables:{topicSlug: queryFieldsObj.topicSlug, pageNumber:queryFieldsObj.pageNumber, pageSize: queryFieldsObj.pageSize}
          })
        }
      })
    } catch(e){
