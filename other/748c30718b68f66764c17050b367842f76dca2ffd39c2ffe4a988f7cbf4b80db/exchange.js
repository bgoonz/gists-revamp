...
export default {
  ...
  getters: {
    currentPage(state) {
      if (!state.pagination.previousFirstItems || state.pagination.previousFirstItems.length === 0) {
        return 1
      }
    ...
    filteredExchanges: state => searchedTitle => {
      ...
      const filteredExchanges = items.filter(item => {
        return item.brand && item.brand.toLowerCase().includes(searchedTitle.toLowerCase()) ||
               item.description && item.description.toLowerCase().includes(searchedTitle.toLowerCase()) ||
               item.category && item.category.toLowerCase().includes(searchedTitle.toLowerCase()) ||
               item.size && item.size.toLowerCase().includes(searchedTitle.toLowerCase()) ||
               item.district && item.district.toLowerCase().includes(searchedTitle.toLowerCase())
    ...
    createExchange({rootState, commit}, exchangeData) {
      ...
      const uploadImageToStorage = (docRefId, imagesToUpload) => {
        ...
        for (const img of imagesToUpload) {
          const storageRef = storage.ref('exchange/' + docRefId + '/' + img.name)
