...
class GameContainer extends ContainerBase {
	...
	render() {
		...
		if (opJoinGame.inProgress) {
			...
			body = <section className="notice error"><p> Game doesnt exist!</p></section>;
			showChat = false;
		...
		return (
			<div className="c-game">
				{body}
				{!showChat ? null :
					<Chat messages={game.messages} opSendMessage = {opSendMessage} sendMessage ={this._sendMessage}></Chat>}
			</div>
		);
	}
}
