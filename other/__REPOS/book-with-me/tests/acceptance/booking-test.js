  ...
  saveBooking(booking) {
    if (booking.start_at && booking.end_at) {
        return new Promise((resolve) => {
          resolve(server.create('booking', {start_at: booking.start_at, end_at: booking.end_at, userId: booking.user.id, rentalId: booking.rental.id}));
        });
    }
  }
});
...
moduleForAcceptance('Acceptance | book rental', {
  beforeEach: function() {
    ...
    this.application.inject('component', 'bookingService', 'service:bookingMock');

    server.create('user', {username: "Ted Mosby"});
    const rentalUser = server.create('user', {username: "Rental Creator"});
    const rental = server.create('rental', {city: 'New York', title: 'City with bookings 1', userId: rentalUser.id});
    ...
    server.create('booking', {start_at: this.startDate, end_at: this.endDate, userId: rentalUser.id, rentalId: rental.id});
  }
});
...
test('should show error if user is not logged in ', async function(assert) {
  ...
  later(async () => {
    ...
    later(async () => {
      ...
      await $("input[name='daterangepicker_start']").val(this.proposedStartDate);
