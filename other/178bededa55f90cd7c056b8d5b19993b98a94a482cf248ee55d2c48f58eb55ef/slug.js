...
import {useGetTopicsBySlug,useGetPostsByTopic,useGetUser,useCreatePost} from "../../../apollo/actions";
...
const PostsPage=withAuth(()=>{
  ...
  const [pagination,setPagination]=useState({pageNumber:parseInt(pageN,10),pageSize:parseInt(pageS,10)})
  const {pageNumber,pageSize}=pagination

  const {data:userData}=useGetUser()
  const user=userData && userData.user || null

  const {data:topicData}=useGetTopicsBySlug({variables:{topicSlug:slug}})
  const topicBySlug=topicData && topicData.topicBySlug || {}
  const {title}=topicBySlug

  const {data:postsData,fetchMore}=useGetPostsByTopic({variables:{topicSlug:slug,...pagination}})
  const postsAndCount=postsData && postsData.postsByTopic||{posts:[],count:0} //postsAndCount contains posts and count as an object. posts initially is empty array
    ...
    <BaseLayout>
      ...
      <Posts
        ...
        onPageChange={(pageNumber,pageSize)=> {
          router.push('/forum/topics/[slug]',`/forum/topics/${slug}?pageN=${pageNumber}&pageS=${pageSize}`,{shallow:true});
          setPagination({pageNumber,pageSize})
        }}
      />
    </BaseLayout>
  )
},['admin','instructor','guest'],{ssr:true})



const Posts=({topic,posts,count,user, topicSlug, fetchMore,pageNumber,pageSize,onPageChange})=>{
  ...
  const [createPost,{error}]=useCreatePost({topicSlug, pageNumber: lastPage, pageSize })
  ...
  const handleCreatePost = async (reply, resetReplier) => {
    ...
    (lastPage === pageNumber || replyTo) && await fetchMore({
  ...
  return (
    <section className="mb-5">
      <div className="fj-post-list">
        {
          topic._id && pageNumber===1 && (                                   //only show the main topic in the first page
            <PostItem
              post={topic}
              ...
              <div className="row" key={post._id}>
                <div className="col-md-9">
                  <PostItem
                    ...
                    onReply={(postData)=>{                                  //when reply button is clicked, it will hold the post data.
