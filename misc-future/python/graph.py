...
def draw_block(graph, tres, id, style, width):
    ...
    if id in graph.walls:
        block = "#" * width
    if id not in graph.walls and random.randint(1, 101) > 99:
        block = (
            "T"
        )  ## tried to do colored blocks but it messes with the data ## colored('T', 'green')
    return block
...
def draw_maze(maze, tres, width=2, **style):
    for y in range(maze.height):
        for x in range(maze.width):
            print(
                "%%-%ds" % width % draw_block(maze, tres, (x, y), style, width), end=""
            )
        print()
...
draw_maze(
    maze2, width=3, path=path_fixer(came_from, start=(1, 4), goal=(7, 8)), tres=[]
)
