...
if (typeof jwplayer == "undefined") {
    ...
    b.utils.stretch = function (h, n, m, f, l, g) {
      ...
      switch (h.toUpperCase()) {
        ...
        case b.utils.stretching.EXACTFIT:
          b.utils.transform(
            n,
            ["scale(", d, ",", e, ")", " translate(0px,0px)"].join("")
          );
          c.width = l;
          c.height = g;
          break;
  ...
  (function (a) {
    ...
    a.plugins.plugin = function (b) {
      var d = "//plugins.longtailvideo.com";
        ...
        if (h.getStatus() == a.utils.loaderstatus.COMPLETE) {
          for (var m = 0; m < e.modes.length; m++) {
            if (e.modes[m].type && a.embed[e.modes[m].type]) {
              ...
              var l = new a.embed[e.modes[m].type](
          ...
          new a.embed.logo(
            a.utils.extend({ hide: true }, e.components.logo),
            "none",
            g.id
          );
        }
  ...
  (function (e) {
    ...
    function c(l) {
      var k = false;
      k =
        l instanceof Array || (typeof l == "object" && !l.position && !l.size);
      return k;
    }
    function j(k) {
      if (typeof k == "string") {
    ...
    a.embed.html5 = function (c, g, b, d, f) {
      function e(j, k, h) {
        return function (l) {
          ...
          var n = m.style;
          j.resize(
            parseInt(n.width.replace("px", "")),
            parseInt(n.height.replace("px", ""))
          );
          k.left = n.left;
          k.top = n.top;
        };
  ...
  (function (a) {
    a.embed.logo = function (m, l, d) {
      var j = {
        prefix: "//l.longtailvideo.com/" + l + "/",
        file: "logo.png",
        link: "//www.longtailvideo.com/players/jw-flv-player/",
        margin: 8,
        out: 0.5,
        over: 1,
        timeout: 5,
  ...
  (function (b) {
    ...
    b.html5.view = function (r, q, f) {
      ...
      function z() {
        ...
        function E() {
          if (
            u.skin.getComponentSettings("display") &&
            u.skin.getComponentSettings("display").backgroundcolor
          ) {
      ...
      function k() {
        for (var E = 0; E < x.plugins.order.length; E++) {
          ...
          if (d.exists(x.plugins.object[F].getDisplayElement)) {
            x.plugins.object[F].height = d.parseDimension(
              x.plugins.object[F].getDisplayElement().style.height
            );
            x.plugins.object[F].width = d.parseDimension(
              x.plugins.object[F].getDisplayElement().style.width
            );
      ...
      function m(E) {
        c(s, { display: x.getMedia().hasChrome() ? "none" : "block" });
      }
      function v(F) {
        ...
        if (d.exists(H)) {
          ...
          for (var E = 0; E < x.plugins.order.length; E++) {
            ...
            if (d.exists(x.plugins.object[G].getDisplayElement)) {
              x.plugins.config[G].currentPosition =
                x.plugins.config[G].position;
            }
          }
        }
        j(x.width, x.height);
      ...
      function h(E) {
        switch (E.keyCode) {
          ...
          case 32:
            if (
              u.jwGetState() != b.api.events.state.IDLE &&
              u.jwGetState() != b.api.events.state.PAUSED
            ) {
              u.jwPause();
            } else {
              u.jwPlay();
      ...
      function o(J, G, H) {
        ...
        for (var E = 0; E < G.length; E++) {
          ...
          if (d.exists(x.plugins.object[K].getDisplayElement)) {
            if (
              x.plugins.config[K].currentPosition != b.html5.view.positions.NONE
            ) {
              var I = J(K, D--);
              if (!I) {
                F.push(K);
              ...
              c(x.plugins.object[K].getDisplayElement(), { display: "none" });
            }
          }
        }
        return F;
      ...
      function l(G, H) {
        ...
        if (typeof x.width == "string" && x.width.lastIndexOf("%") > -1) {
          percentage =
            parseFloat(x.width.substring(0, x.width.lastIndexOf("%"))) / 100;
          E = Math.round(window.innerWidth * percentage);
        }
        if (typeof x.height == "string" && x.height.lastIndexOf("%") > -1) {
          percentage =
            parseFloat(x.height.substring(0, x.height.lastIndexOf("%"))) / 100;
        ...
        return {
          ...
          width:
            E -
            d.parseDimension(s.style.left) -
            d.parseDimension(s.style.right),
          height:
            F -
            d.parseDimension(s.style.top) -
            d.parseDimension(s.style.bottom),
          zIndex: H,
        };
      }
      function B(E, F) {
      ...
      function y() {
        ...
        if (H && H.tagName.toLowerCase() == "video") {
          ...
          var E, I;
          if (
            s.style.width.toString().lastIndexOf("%") > -1 ||
            s.style.width.toString().lastIndexOf("%") > -1
          ) {
          ...
          d.stretch(
            ...
            E,
            I,
            H.videoWidth ? H.videoWidth : 400,
            H.videoHeight ? H.videoHeight : 300
          );
        } else {
          var G = x.plugins.object.display.getDisplayElement();
          if (G) {
            x.getMedia().resize(
              d.parseDimension(G.style.width),
              d.parseDimension(G.style.height)
            );
          } else {
            x.getMedia().resize(
              d.parseDimension(s.style.width),
              d.parseDimension(s.style.height)
            );
          }
        }
      }
      function e(F) {
        var G = { position: "absolute", margin: 0, padding: 0, top: null };
        ...
        switch (E.toUpperCase()) {
          case b.html5.view.positions.TOP:
            ...
            s.style[E] =
              d.parseDimension(s.style[E]) + x.plugins.object[F].height + "px";
          ...
          case b.html5.view.positions.RIGHT:
            ...
            G.height =
              ...
              d.parseDimension(s.style.bottom);
            s.style[E] =
              d.parseDimension(s.style[E]) + x.plugins.object[F].width + "px";
    ...
    function a(e) {
      return (
        [
          b.html5.view.positions.TOP,
          b.html5.view.positions.RIGHT,
          b.html5.view.positions.BOTTOM,
          b.html5.view.positions.LEFT,
        ]
          .toString()
  ...
  (function (a) {
    ...
    a.html5.controlbar = function (l, V) {
      ...
      var D = _utils.extend(
        {},
        b,
        k.skin.getComponentSettings("controlbar"),
        V
      );
      if (
      ...
      function N() {
        ...
        P = 0;
        if (!p) {
          var ak = { height: J().height, backgroundColor: D.backgroundcolor };
      ...
      function H(ai) {
        return function () {
          if (S && d != ai) {
            d = ai;
            W.sendEvent(ai, { component: "controlbar", boundingRect: O() });
          }
        };
      }
      ...
      function C(am, ao) {
        ...
        switch (am.name) {
          ...
          case "time":
            aj = !_utils.exists(
              k.skin.getSkinElement("controlbar", "timeSliderCapLeft")
            )
              ? 0
              : k.skin.getSkinElement("controlbar", "timeSliderCapLeft").width;
            ak = !_utils.exists(
              k.skin.getSkinElement("controlbar", "timeSliderCapRight")
            )
              ? 0
              : k.skin.getSkinElement("controlbar", "timeSliderCapRight").width;
            al = ao == "left" ? aj : ak;
            ai =
              k.skin.getSkinElement("controlbar", "timeSliderRail").width +
              aj +
              ak;
            aq = {
              height: J().height,
          ...
          case "volume":
            aj = !_utils.exists(
              k.skin.getSkinElement("controlbar", "volumeSliderCapLeft")
            )
              ? 0
              : k.skin.getSkinElement("controlbar", "volumeSliderCapLeft")
                  .width;
            ak = !_utils.exists(
              k.skin.getSkinElement("controlbar", "volumeSliderCapRight")
            )
              ? 0
              : k.skin.getSkinElement("controlbar", "volumeSliderCapRight")
                  .width;
            al = ao == "left" ? aj : ak;
            ai =
              k.skin.getSkinElement("controlbar", "volumeSliderRail").width +
              aj +
              ak;
            aq = {
              height: J().height,
      ...
      function x(al, ao, aj, ar, am, ai, ak) {
        if (
          _utils.exists(k.skin.getSkinElement("controlbar", al)) ||
          al.indexOf("Text") > 0 ||
          al.indexOf("divider") === 0
        ) {
          var an = {
            height: J().height,
            position: "absolute",
          ...
          if (
            (al.indexOf("next") === 0 || al.indexOf("prev") === 0) &&
            k.jwGetPlaylist().length < 2
          ) {
            aj = false;
            an.display = "none";
          }
            ...
            if (al.indexOf("divider") === 0) {
              if (ai) {
                ...
                if (ak) {
                  ...
                  an.background =
                    "url(" +
                    k.skin.getSkinElement("controlbar", "divider").src +
              ...
              an.background =
                "url(" +
                k.skin.getSkinElement("controlbar", al).src +
                ") repeat-x center left";
          ...
          if (p) {
            ...
            var aq = Z(al, ar, an);
            if (
              _utils.exists(k.skin.getSkinElement("controlbar", al + "Over"))
            ) {
              aq.onmouseover = function (au) {
                aq.style.backgroundImage = [
                  "url(",
                  k.skin.getSkinElement("controlbar", al + "Over").src,
                  ")",
                ].join("");
              };
      ...
      function ae() {
        I({ id: k.id, duration: k.jwGetDuration(), position: 0 });
        u({ id: k.id, bufferProgress: 0 });
        ag({ id: k.id, mute: k.jwGetMute() });
        r({ id: k.id, newstate: a.api.events.state.IDLE });
        m({ id: k.id, volume: k.jwGetVolume() });
      }
      function R(ak, al, aj) {
        if (p) {
          return;
      ...
      function X(ai) {
        ...
        aj.onmousemove = function (ak) {
          if (v == "time") {
            ...
            _css(Q.timeSliderThumb, { left: al });
          }
        };
      }
      function af(aj) {
      ...
      function u(aj) {
        ...
        if (c.timeSliderRail) {
          ...
          var ai = isNaN(Math.round((ak * f) / 100))
            ? 0
            : Math.round((ak * f) / 100);
          _css(Q.timeSliderBuffer, { width: ai });
        }
      }
      function ag(ai) {
        if (ai.mute) {
      ...
      function r(ai) {
        if (
          ai.newstate == a.api.events.state.BUFFERING ||
          ai.newstate == a.api.events.state.PLAYING
        ) {
          _show(Q.pauseButton);
          _hide(Q.playButton);
        } else {
      ...
      function L(ai) {
        u({ bufferPercent: 0 });
        I(_utils.extend(ai, { position: 0, duration: ad }));
      }
      function I(al) {
        ...
        if (am) {
          var ai = isNaN(Math.round(am.width * aj))
            ? 0
            : Math.round(am.width * aj);
          var ak = ai;
      ...
      function n() {
        ...
        for (var ai in ak.childNodes) {
          ...
          if (
            al[ai].id.indexOf(ac.id + "_divider") === 0 &&
            aj &&
            aj.id.indexOf(ac.id + "_divider") === 0 &&
            al[ai].style.backgroundImage == aj.style.backgroundImage
          ) {
      ...
      function w() {
        ...
        if (_utils.exists(Q.timeSliderCapRight)) {
          _css(Q.timeSliderCapRight, { left: am + (ai.width - ab - E) });
        }
        _css(ac, aj);
        _css(Q.elements, ai);
      ...
      function m(am) {
        if (_utils.exists(Q.volumeSliderRail)) {
          ...
          var aj = !_utils.exists(
            k.skin.getSkinElement("controlbar", "volumeSliderCapLeft")
          )
            ? 0
            : k.skin.getSkinElement("controlbar", "volumeSliderCapLeft").width;
          _css(Q.volumeSliderProgress, { width: ai, left: aj });
          if (_utils.exists(Q.volumeSliderCapLeft)) {
            _css(Q.volumeSliderCapLeft, { left: 0 });
          }
        }
      }
      function t() {
  ...
  (function (b) {
    ...
    b.html5.controller = function (z, w, h, v) {
      ...
      var e = -1;
      var A =
        c.exists(G.config.debug) &&
        G.config.debug.toString().toLowerCase() == "console";
      ...
      function f(M) {
        ...
        if (G.item != M) {
          G.item = M;
          J = true;
          m.sendEvent(b.api.events.JWPLAYER_PLAYLIST_ITEM, { index: M });
        }
      }
      function H(N) {
        try {
      ...
      function F(M) {
        return function () {
          if (d) {
            ...
            p.push({ method: M, arguments: arguments });
          }
        };
      }
      function x(O, N) {
  ...
  (function (a) {
    ...
    a.html5.display = function (k, G) {
      ...
      var D = {
        ...
        display_iconBackground: {
          style: {
            ...
            position: "absolute",
            top:
              (u - h.skin.getSkinElement("display", "background").height) / 2,
            left:
      ...
      this.resize = function (S, R) {
        _css(P.display, { width: S, height: R });
        _css(P.display_text, {
          width: S - 10,
      ...
      function O(R) {
        ...
        P.display_icon.style.backgroundImage = [
          "url(",
          h.skin.getSkinElement("display", R).src,
          ")",
        ].join("");
        _css(P.display_icon, {
          ...
          top:
            (h.skin.getSkinElement("display", "background").height -
              h.skin.getSkinElement("display", R).height) /
            2,
          left:
            (h.skin.getSkinElement("display", "background").width -
              h.skin.getSkinElement("display", R).width) /
            2,
        });
        b();
        if (_utils.exists(h.skin.getSkinElement("display", R + "Over"))) {
          P.display_icon.onmouseover = function (S) {
            P.display_icon.style.backgroundImage = [
              "url(",
              h.skin.getSkinElement("display", R + "Over").src,
              ")",
            ].join("");
          };
      ...
      function o(R) {
        ...
        P.display_text.style.top =
          (u - P.display_text.getBoundingClientRect().height) / 2 + "px";
      }
      function E() {
      ...
      function p(R) {
        if (
          (R.type == a.api.events.JWPLAYER_PLAYER_STATE ||
            R.type == a.api.events.JWPLAYER_PLAYLIST_ITEM) &&
          A
        ) {
          A = false;
      ...
      function v() {
        if (
          ...
          _css(P.display_image, { display: "block" });
          P.display_image.src = _utils.getAbsolutePath(
            h.jwGetPlaylist()[h.jwGetItem()].image
          );
        }
      }
    ...
    a.html5.dock = function (p, u) {
          ...
          if (x) {
            ...
            if (!f[A].div) {
              ...
              if (p.skin.getSkinElement("dock", "button")) {
                f[A].div.childNodes[1].src = p.skin.getSkinElement(
                  "dock",
                  "button"
                ).src;
              }
      ...
      function h(x, J) {
        if (s.length > 0) {
          ...
          for (var z = 0; z < s.length; z++) {
            ...
            A.style.left =
              C +
              (p.skin.getSkinElement("dock", "button").width + y) * K * F +
              "px";
            var D = {
      ...
      function b(x) {
        return function () {
          if (!d && r != x && s.length > 0) {
            r = x;
            j.sendEvent(x, { component: "dock", boundingRect: e });
          }
        };
      }
      function l(x) {
  ...
  (function (a) {
    var b = {
      prefix: "//l.longtailvideo.com/html5/",
      file: "logo.png",
      link: "//www.longtailvideo.com/players/jw-flv-player/",
      margin: 8,
      out: 0.5,
  ...
  (function (a) {
    ...
    a.html5.mediavideo = function (h, s) {
      ...
      this.seek = function (I) {
        if (
          !(y.duration <= 0 || isNaN(y.duration)) &&
          !(y.position <= 0 || isNaN(y.position))
        ) {
          m.currentTime = I;
          m.play();
        }
      ...
      function D(J) {
        if (e.exists(J) && e.exists(J.target)) {
          if (
            !isNaN(J.target.duration) &&
            (isNaN(y.duration) || y.duration < 1)
          ) {
          ...
          if (B == a.api.events.state.PLAYING) {
            ...
            y.position =
              y.duration > 0 ? Math.round(J.target.currentTime * 10) / 10 : 0;
      ...
      function G(I) {
        ...
        if (
          (y.duration === 0 || isNaN(y.duration)) &&
          I.target.duration != Infinity
        ) {
          y.duration = Math.round(I.target.duration * 10) / 10;
        }
        j.sendEvent(a.api.events.JWPLAYER_MEDIA_META, { metadata: J });
      }
      function u(K) {
        ...
        if (
          (K.target.error && K.target.tagName.toLowerCase() == "video") ||
          (K.target.parentNode.error &&
            K.target.parentNode.tagName.toLowerCase() == "video")
        ) {
          var I = !e.exists(K.target.error)
            ? K.target.parentNode.error
            : K.target.error;
          switch (I.code) {
    ...
    a.html5.playlistloader = function () {
      ...
      function b(e) {
        c.sendEvent(a.api.events.JWPLAYER_ERROR, {
          error: e
            ? e
            : "could not load playlist for whatever reason.  too bad",
        });
      }
    };
  })(jwplayer);
  ...
  (function (a) {
    a.html5.api = function (c, n) {
      ...
      function e() {
        if (
          l.state == a.api.events.state.PLAYING ||
...
function scrollRefresh(anchorTag) {
  $("#" + anchorTag.substring(1)).jScrollPane();
}
...
$(document).ready(function () {
  $("a.poplight").click(function () {
    ...
    //Fade in the Popup and add close button. Image src will start from /images in mirror version.
    $("#" + popID)
      .fadeIn()
      .css({ width: Number(popWidth) })
      .prepend(
        '<a href="#" class="close"><img src="/images/close.png" class="video_popup_btn_close" title="Close Window" alt="Close" /></a>'
      );
    ...
    var interactiveTranscriptContainer = $(this)
      .attr("onclick")
      .split(",")[1]
      .split("'")[1];
    interactiveTranscriptDivName =
      interactiveTranscriptContainer + "transcript1";
    ...
    if ($("#" + interactiveTranscriptDivName).length > 0) {
      ...
      plugin_params[interactiveTranscriptContainer] =
        threePlayParams[interactiveTranscriptContainer];
      plugin_params[interactiveTranscriptContainer]["transcript"][
        "height"
      ] = 140;
      three_play_obj = $(document)[0].defaultView.P3;
      if (three_play_obj != undefined) {
        three_play_obj.init(plugin_params, "");
      }
    }
  ...
  $("a.close, #video_popup_fade").live("click", function () {
...
function initialize_video_wrapper_markup(theWrapper) {
  // Live version
  if (oldWrapper != null) {
    reset_video_wrapper_markup(oldWrapper, oldCode);
  }
...
function load_media_chapter(
  ...
  var options = set_jwplayer_options(
    "embed1",
    "player21",
    media_url,
    provider,
  ...
  //set duration of the jwplayer = stop time when start and end time of the scholar Chapter Embed is given
  if (stop != 0) {
    options["duration"] = stop;
  }

  if (provider == "rtmp") {
    options = set_jwplayer_options_techtv(options);
  }

  jwplayer("embed1").setup(options);
}

function ocw_embed_media(
  container_id,
  media_url,
  provider,
  page_url,
  ...
  jwplayer(container_id).setup(options);
}

// This function is used to embed chaptered media on pages
function ocw_embed_chapter_media(
  container_id,
  media_url,
  provider,
  page_url,
...
function load_multiple_media_chapter(
  theWrapper,
  container_id,
  thePlayer,
  media_url,
  ...
  //set duration of the jwplayer = stop time when start and end time of the scholar popup is given
  if (stop != 0) {
    options["duration"] = stop;
  }
...
function HFH_video_popup(
  ...
  //set the additional options for TechTV HFH videos
  options = set_jwplayer_options_techtv(options);
...
function video_popup(
  ...
  //set duration of the jwplayer = stop time when start and end time of the inline pop-up is given
  if (stop != 0) {
    options["duration"] = stop;
  }

  // set the additional options for TechTV videos
  if (provider == "rtmp") {
    options = set_jwplayer_options_techtv(options);
  }
...
function set_jwplayer_options(
  ...
  // Set the provider to video for mirror version streaming
  if (provider == "youtube" || provider == "http" || provider == "rtmp") {
    provider = "video";
  }
  ...
  if (captions_file != null && captions_file != "") {
    plugin_list["/jw-player-free/captions-2.0h.swf"] = { file: captions_file };
  }
  ...
  //Using jw-player-free components in offline version
  return {
    players: [
      { type: "flash", src: "/jw-player-free/player.swf" },
      { type: "html5" },
    ],
    primary: "html5",
...
function set_jwplayer_options_techtv(options) {
  //set the configuration options specific to TechTV HFH videos.
  options["streamer"] = "rtmp://s1cgf0x4vkgx2w.cloudfront.net/cfx/st/";
  options["http.startparam"] = "start";
