...
// array sorter function this will take in an array and sort it
function arraySorter(arr) {
  ...
  for (x = 0; x < count; x++) {
    for (y = 0; y < count; y++) {
      if (arr[y] > arr[y + 1]) {
        // TODO: refactur in to its own function swap()
        temp = arr[y + 1];
        arr[y + 1] = arr[y];
        arr[y] = temp;
      }
...
let carYears = [];
carYears = extractYears(inventory);
...
let oldCars = carsOlderThan2k(carYears);
