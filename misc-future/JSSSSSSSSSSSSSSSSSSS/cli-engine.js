...
function processText(
  ...
  const fixedResult = linter.verifyAndFix(text, config, {
    ...
    postprocess:
      processor &&
      ((problemLists) =>
        processor.postprocess(problemLists, effectiveFilename)),
  });
...
class CLIEngine {
  ...
  constructor(providedOptions) {
    ...
    if (
      this.options.fix &&
      this.options.fixTypes &&
      this.options.fixTypes.length > 0
    ) {
      ...
      const originalFix =
        typeof this.options.fix === "function"
          ? this.options.fix
          : () => this.options.fix;
  ...
  executeOnFiles(patterns) {
    ...
    const results = fileList.map((fileInfo) => {
      ...
      const { result, config } = processFile(
        fileInfo.filename,
        configHelper,
        options,
        this.linter
    ...
    const usedDeprecatedRules = createRuleDeprecationWarnings(
      allUsedRules,
      this.getRules()
    );
  ...
  executeOnText(text, filename, warnIgnored) {
    ...
    if (resolvedFilename && ignoredPaths.contains(resolvedFilename)) {
      ...
      usedDeprecatedRules = createRuleDeprecationWarnings(
        Object.keys(config.rules).filter((rule) =>
          ConfigOps.getRuleSeverity(config.rules[rule])
        ),
        this.getRules()
      );
    }
