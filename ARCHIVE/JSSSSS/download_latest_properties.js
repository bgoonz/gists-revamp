...
request(url, function (error, response, body) {
  if (!error && response.statusCode === 200) {
    ...
    var workingDraftAndOlderProperties = allCSSProperties.filter(function (
      cssProp
    ) {
    ...
    var out_file = fs.createWriteStream(
      path.resolve(__dirname, "./../lib/allProperties.js"),
      {
        encoding: "utf-8",
      }
    );
    ...
    CSSpropertyNames.forEach(function (property) {
      out_file.write(
        "allProperties.add(" + JSON.stringify(camelToDashed(property)) + ");\n"
      );
    });
    ...
    out_file.end(function (err) {
      ...
      console.log(
        "Generated " + Object.keys(CSSpropertyNames).length + " properties."
      );
    });
  } else {
    throw error;
