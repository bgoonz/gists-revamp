...
function makeIndent(arg, deflen, name) {
  if (arg === null || arg === undefined) return space(deflen);
  else if (typeof arg === "number") return space(arg);
  else if (typeof arg === "string") return arg;
  else assert.fail('invalid "' + name + '": not a string or number: ' + arg);
}
  ...
  outer: while (i < args.length) {
    ...
    if (arg === "--") {
      ...
      if (!option) {
        ...
        else if (this.interspersed) _args.push(arg);
        else break outer;
      } else {
        ...
        if (!takesArg) {
          ...
          throw new Error(
            format('do not have enough args for "--%s" ' + "option", name)
          );
        } else {
      ...
      while (j < arg.length) {
        ...
        if (!option) {
          ...
          if (this.allowUnknown) {
            ...
            throw new Error(
              format('unknown option: "-%s" in "%s" group', name, arg)
            );
          } else {
            throw new Error(format('unknown option: "-%s"', name));
          }
...
function bashCompletionSpecFromOptions(args) {
  ...
  spec.push(
    format('local cmd%s_shortopts="%s"', context, shortopts.sort().join(" "))
  );
  spec.push(
  ...
  if (args.argtypes) {
    spec.push(
      format('local cmd%s_argtypes="%s"', context, args.argtypes.join(" "))
    );
  }
  return spec.join("\n");
}
