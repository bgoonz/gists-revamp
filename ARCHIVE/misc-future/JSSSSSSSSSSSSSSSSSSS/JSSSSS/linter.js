...
function addDeclaredGlobals(globalScope, configGlobals, commentDirectives) {
  const mergedGlobalsInfo = Object.assign(
    {},
    lodash.mapValues(configGlobals, (value) => ({
      sourceComment: null,
      value: ConfigOps.normalizeConfigGlobal(value),
    })),
    lodash.mapValues(
      commentDirectives.enabledGlobals,
      ({ comment, value }) => ({
      ...
      if (!variable) {
        ...
        if (mergedGlobalsInfo[name].sourceComment === null) {
          ...
          variable.eslintExplicitGlobalComment =
            mergedGlobalsInfo[name].sourceComment;
        }
...
function getDirectiveComments(filename, ast, ruleMapper) {
  ...
  ast.comments
    ...
    .forEach((comment) => {
      ...
      const match = /^(eslint(-\w+){0,3}|exported|globals?)(\s|$)/u.exec(
        trimmedCommentText
      );
