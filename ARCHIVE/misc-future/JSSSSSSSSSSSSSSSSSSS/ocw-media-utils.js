    ...
    $("#" + popID)
      ...
      .prepend(
        '<a href="#" class="close"><img src="' +
          close_btn +
          '" class="video_popup_btn_close" title="Close Window" alt="Close" /></a>'
      );
...
$(document).ready(function () {
  ...
  $("#cc-control").click(function () {
    if (cc_on == 0) {
      $("#subtitles").animate(
        { height: "49px", opacity: 5 },
        { queue: false, duration: 500 }
      );
      cc_on = 1;
      document.getElementById("captions").style.backgroundImage =
        "url(/images/cc_on.PNG)";
    } else {
      $("#subtitles").animate(
...
function get_subtitle(position) {
  for (i = 1; i <= sub_titles.length; i++) {
    if (
      sub_titles[i]["start"] <= position &&
      sub_titles[i]["end"] >= position
    ) {
...
function timeMonitor(obj) {
  ...
  if (document.getElementById("subtitles") != null) {
    document.getElementById("subtitles").innerHTML = get_subtitle(
      current_position
    ).replace("\n", "<br>");
  }
}
...
function change(path, page, container_id, relative) {
  if (relative) {
    if (prev_id != container_id) {
      last =
        parseInt(document.getElementById("numbers_" + container_id).innerHTML) -
        1;
      prev_id = container_id;
    }
    page += last;
  ...
  document.getElementById("annots_" + container_id).innerHTML =
    annots[page] || "";
...
function back5Seconds() {
  jwplayer(player).seek(current_position - 6);
}
...
function setMultipleCaptionsOld(captions) {
  //Patch to add multiple caption for existing videos, needs to be removed before Full Site Republish
...
function set_jwplayer_events(jw_player, container_id, start) {
  jw_player.onReady(function () {
    ...
    if (start != 0) {
      jw_player.onPlay(function () {
        if (isFirstPositionSet == false) {
          jw_player.seek(firstPosition);
...
function get_modified_id(container_id) {
  ...
  if (index > -1) {
    container_id = container_id.substring(
      0,
      index ? index : container_id.length
    );
  }
  return container_id;
}
...
function get_caption_details(container_id) {
  ...
  if (document.getElementById("caption_" + container_id) != null) {
    //Patch to add multiple caption for existing videos, needs to be removed before Full Site Republish
    var captions_info = document.getElementById(
      "caption_" + container_id
    ).innerHTML;
    var caption_url = setMultipleCaptionsOld(captions_info.replace(/'/gi, '"'));
    return caption_url;
  }
...
function set_jwplayer_options(
  ...
  if (provider == "rtmp") {
    ...
    sharing_embed_prefix +=
      "https://techtv.mit.edu/embeds/" +
      techTvId +
      "?size=large&amp;custom_width=530&amp;custom_height=325&amp;";
    ...
    var sharing_embed_string =
      sharing_embed_prefix + media_url + sharing_embed_suffix;
  }

  var sharing_params = {
    link: "https://ocw.mit.edu" + page_url,
