...
$(document).ready(function () {
  ...
  if (
    threePlayScriptTags.length == Object.keys(threePlayParams).length &&
    Object.keys(threePlayParams).length > 0
  ) {
...
var sponsors_image_array = {
  img0: [
    "https://ocw.mit.edu/images/logo_mathworks.png",
    "Mathworks logo.",
    "http://www.mathworks.com/",
  ],
  img1: [
    "https://ocw.mit.edu/images/logo_accenture.png",
    "Accenture logo.",
    "http://careers.accenture.com/",
  ],
  img2: [
    "https://ocw.mit.edu/images/logo_lockheed.png",
    "Lockheed Martin logo.",
    "http://www.lockheedmartin.com/",
  ],
  img3: [
    "https://ocw.mit.edu/images/logo_dow.png",
    "Dow logo.",
    "http://www.dow.com/",
  ],
  img4: [
    "https://ocw.mit.edu/images/logo_abinitio.png",
    "Ab Initio logo.",
    "http://www.abinitio.com/",
  ],
  img5: [
    "https://ocw.mit.edu/images/logo_telmex.png",
    "Telmex logo.",
...
function shuffle_sponsor_array(sponsors_image_array, len) {
  ...
  while (i--) {
    ...
    shuffled_sponsors_array["img" + i] =
      shuffled_sponsors_array["img" + random_position_no]; //  Fisher Yates algorithm
...
function shuffled_sponsors_markup() {
  ...
  shuffled_sponsors_image_array = shuffle_sponsor_array(
    sponsors_image_array,
    len
  );
  ...
  for (var i = 0; i < len; i++) {
    if (i % 2 == 0) {
      // Condition for checking whether logo will appear on left or right side and based on that inline styles will be applied.
      if (shuffled_sponsors_image_array["img" + i][2] == "") {
        // Checking whether logo has any anchor tag or not
        shuffled_sponsors_html =
          shuffled_sponsors_html +
          '<img alt="' +
          shuffled_sponsors_image_array["img" + i][1] +
          '" src="' +
          shuffled_sponsors_image_array["img" + i][0] +
          '" style="height: 53px; width: 145px; margin: 10px 10px 10px 0;" />';
      } else {
        shuffled_sponsors_html =
          shuffled_sponsors_html +
          '<a href="' +
          shuffled_sponsors_image_array["img" + i][2] +
          '"><img alt="' +
          shuffled_sponsors_image_array["img" + i][1] +
          '" src="' +
          shuffled_sponsors_image_array["img" + i][0] +
          '" style="height: 53px; width: 145px; margin: 10px 10px 10px 0;" /></a>';
      }
    } else {
      if (shuffled_sponsors_image_array["img" + i][2] == "") {
        shuffled_sponsors_html =
          ...
          '" src="' +
          shuffled_sponsors_image_array["img" + i][0] +
          '" style="height: 53px; width: 145px; margin: 10px 0;" /><br />';
      } else {
        shuffled_sponsors_html =
          shuffled_sponsors_html +
          '<a href="' +
          ...
          '" src="' +
          shuffled_sponsors_image_array["img" + i][0] +
          '" style="height: 53px; width: 145px; margin: 10px 0;" /></a><br />';
      }
    }
  }
  $("#foot-c5").empty();
...
function createThreePlayParams(project_id, video_id, container, start, end) {
  ...
  if (container == "embed_1") {
    target = "transcript1";
  } else {
    target = container + "transcript1";
  }
...
window.newWindow = function (html, eqnstr) {
  if (windowRef[eqnstr] != undefined && !windowRef[eqnstr].closed) {
    windowRef[eqnstr].close();
  }
...
function numericTypedOrDropDownSelected(qid) {
  //For numericalresponse and optionresponse problems
...
function optionSelected(qid) {
  //for multiplechoiceresponse and choiceresponse problems
  var slides = document.getElementsByName("Q" + qid + "_input");
  for (var i = 0; i < slides.length; i++) {
    var status_p = document.getElementById(
      slides.item(i).id + "_normal_status"
    );
    status_p.className = "nostatus";
    var aria_status_p = document.getElementById(
      slides.item(i).id + "_aria_status"
    );
  ...
  var status_p_combined = document.getElementById(
    "Q" + qid + "_status_combined"
  );
...
function checkNumericalResponse(usr_input, ans, tolerance) {
  //ans should be in range
  if (
    (usr_input >= ans && usr_input <= ans + tolerance) ||
    (usr_input <= ans && usr_input >= ans - tolerance)
  ) {
    return true;
  }
...
function showHideSolution(
  ...
  for (var qid in prob_id_type_array) {
    ...
    if (type == "numerical") {
      ...
      ansp.innerHTML =
        "Solution: " + document.getElementById("Q" + qid + "_ans").value;
    ...
    if (type == "choiceresponse" || type == "multiple_choice") {
      ...
      for (var i = 0; i < checkboxes.length; i++) {
        if (checkboxes[i].getAttribute("correct") == "true") {
          var status_p = document.getElementById(
            checkboxes[i].id + "_normal_status"
          );
          status_p.className = statusClass;
          var aria_status_p = document.getElementById(
            checkboxes[i].id + "_aria_status"
          );
...
function checkAnswer(prob_id_type_array) {
  ...
  for (var qid in prob_id_type_array) {
    ...
    if (type == "stringresponse") {
      ...
      status_p = document.getElementById("Q" + qid + "_normal_status");
      aria_status_p = document.getElementById("Q" + qid + "_aria_status");
      if (input_focus.getAttribute("ckecktype").indexOf("regexp") > -1) {
        //regex case
      } else {
        correct = input_focus.value == input_focus.getAttribute("answer");
      }
    }
    ...
    if (type == "multiple_choice") {
      ...
      for (var i = 0; i < options.length; i++) {
        if (options[i].checked) {
          ...
          aria_status_p = document.getElementById(
            options[i].id + "_aria_status"
          );
          input_focus = options[i];
          if (options[i].getAttribute("correct") == "true") {
            correct = true;
          }
    ...
    if (type == "choiceresponse") {
      ...
      for (var i = 0; i < options.length; i++) {
        if (
          (options[i].checked &&
            options[i].getAttribute("correct") == "false") ||
          (!options[i].checked && options[i].getAttribute("correct") == "true")
        ) {
          correct = false;
        }
      }
    ...
    //default is wrong
    if (status_p != null) {
      status_p.className = "wrong";
    }
    if (aria_status_p != null) {
      aria_status_p.innerHTML = "[wrong]";
    }
    if (correct) {
      if (status_p != null) {
        status_p.className = "correct";
      }
      if (aria_status_p != null) {
        aria_status_p.innerHTML = "[correct]";
      }
    }
    if (input_focus != null) {
      input_focus.focus();
...
function image_popup(
  ...
  if (!extended_alt) {
    ...
    screen_reader_div.text(extended_alt);
  }
  ...
  setTimeout(function () {
    document.getElementById("img_popup_close").focus();
  }, 0);

  $("#img_popup_close, #image_popup_fade").on("click", function (event) {
  ...
  $("#popup_image_div").keydown(function (event) {
    trapTabKey($(this), event, focusedElem);
  });
}
$("a.imagePopup").live("keydown", function (event) {
  trapKeys($(this), event);
});

function trapKeys(obj, evt) {
  if (evt.which == 1 || evt.which == 32 || evt.which == 13) {
    //Enter key pressed
