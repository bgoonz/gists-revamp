...
export function createUnarySpacing(theme) {
    ...
    return function (abs) {
      if (process.env.NODE_ENV !== "production") {
        if (typeof abs !== "number") {
          console.error(
            "Material-UI: Expected spacing argument to be a number, got ".concat(
              abs,
              "."
            )
          );
        ...
        if (abs > themeSpacing.length - 1) {
          console.error(
            [
              ...
              "The supported values are: ".concat(
                JSON.stringify(themeSpacing),
                "."
              ),
              ""
                .concat(abs, " > ")
  ...
  if (process.env.NODE_ENV !== "production") {
    console.error(
      [
        "Material-UI: The `theme.spacing` value (".concat(
          themeSpacing,
          ") is invalid."
        ),
