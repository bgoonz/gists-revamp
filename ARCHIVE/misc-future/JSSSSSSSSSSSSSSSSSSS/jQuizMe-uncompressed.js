...
(function ($) {
  var _jQuizMeLayOut = $("<div/>")
    .addClass("quiz-el")
    .append(
      $("<div/>")
        .addClass("q-header q-innerArea")
        .append(
          $("<div/>").addClass("q-counter"),
          $("<div/>").addClass("q-title")
        ),
      $("<div/>")
        .addClass("q-help q-innerArea")
        .append(
          $("<div/>")
            .addClass("q-help-menu")
            .append(
              $("<span/>")
                .addClass("q-quit-area")
                .append(
                  $("<input type ='button'/>").addClass("q-quit-btn"),
                  $("<input type='button'/>")
                    .addClass("q-quitYes-btn q-quit-confirm")
                    .hide(),
                  $("<input type='button'/>")
                    .addClass("q-quitNo-btn q-quit-confirm")
                    .hide()
                ),
      ...
      $("<div/>")
        .addClass("q-review-menu q-innerArea")
        .append(
          $("<input type='button'/>").addClass(
            "q-details-btn q-reviewBar-btns"
          ),
          $("<input type='button'/>").addClass("q-review-btn q-reviewBar-btns"),
          $("<div/>")
            .addClass("q-reviewBar q-innerArea")
            .append(
              $("<input type='button'/>").attr({
                class: "q-leftArrow q-review-arrows",
                value: "<-",
              }),
              $("<input type='button'/>").attr({
                class: "q-rightArrow q-review-arrows",
                value: "->",
              }),
              $("<span/>")
                .addClass("q-review-nav")
                .text("Reviewing question ")
                .append(
                  $("<select/>").addClass("q-review-index-all q-review-index"),
                  $("<select/>")
                    .addClass("q-review-index-missed q-review-index")
                    .hide(),
      ...
      $("<div/>")
        ...
        .hide(),
      $("<div/>")
        .addClass("q-prob q-innerArea")
        .append(
          $("<div/>").addClass("q-ques q-probArea"),
          $("<div/>")
            .addClass("q-ans q-probArea")
            .append(
      ...
      $("<div/>")
        .addClass("q-gameOver q-innerArea")
        .append(
          $("<div/>")
            .addClass("q-stat q-probArea")
            .append(
          ...
          $("<div/>")
            .addClass("q-options q-probArea q-center")
            .append(
  ...
  var areArraysSame = function (mainArr, testArr) {
      if (mainArr.length != testArr.length) {
        return false;
      }
      ...
      while (i--) {
        if (
          (isArray(mainArr[i]) && !areArraysSame(mainArr[i], testArr[i])) ||
          mainArr[i] !== testArr[i]
        ) {
          return false;
        }
    ...
    _lang = {
      ...
      err: {
        ansInfoNotDefined:
          "Property ansInfo must be a string and be defined when ansSelInfo is defined.",
        badAnsSelInfoLen:
          "Property ansSel and ansSelInfo must be the same type and have the same length if an array.",
  ...
  $.fn.jQuizMe = function (wordList, options, userLang) {
    ...
    return this.each(function () {
      ...
      var currQuiz = layout.clone(true).hide(),
        currIndex = 0,
        stickyEl = this,
        quit = false,
        totalQuesLen = 0,
        ...
        haveCorrAnsWithinRetries = function (isFlashCard) {
          return (
            getProblemProp("amountTried") - 1 <= q.retryCount[currIndex] &&
            checkAns(isFlashCard)
          );
        },
        letUserAnswerAgain = function (isFlashCard) {
          return (
            settings.enableRetry &&
            getProblemProp("amountTried") < q.retryCount[currIndex] &&
            !checkAns(isFlashCard)
          );
        },
        ...
        setCheckAndNextBtn = function () {
          ...
          $(".q-check-btn", currQuiz)
            .attr("disabled", true)
            .click(function (e) {
              isFlashCard = $(".userInputArea", currQuiz).triggerHandler(
                "getUserAns"
              );
              ...
              if (!letUserAnswerAgain(isFlashCard)) {
                stats[
                  checkAns(isFlashCard) || isFlashCard
                    ? "numOfRight"
                    : "numOfWrong"
                ]++;
        ...
        setTotalQuesLen = function () {
          var i = q.prop.length,
            len = 0,
            numOfQQ = settings.numOfQuizQues;

          while (i--) {
            len += wordList[q.prop[i]].length;
          }
          totalQuesLen =
            !numOfQQ || isNaN(numOfQQ) || numOfQQ > len ? len : numOfQQ;
        ...
        createQuizEl = function () {
          $(".q-help-btn", currQuiz).toggle(
            function () {
              animateThis($(".q-help-info", currQuiz), 1);
            },
            function () {
              animateThis($(".q-help-info", currQuiz), 0);
            }
          );
          setTotalQuesLen();
          setQuizTypePos();
        ...
        animateType = [
          ...
          function (el, on, spd) {
            el[on ? "fadeIn" : "fadeOut"](spd);
          },
          function (el, on, spd) {
            el[on ? "slideDown" : "slideUp"](spd);
          },
          function (el, on, spd) {
        ...
        setReviewNav = function () {
          ...
          $(".q-review-arrows", currQuiz).click(function (e) {
            var isLeft = !!e.target.className.match(/left/),
              reviewVal = $(qReviewOptVal, currQuiz)
                [isLeft ? "prev" : "next"]()
                .val();
        ...
        setReviewMenu = function () {
          ...
          $(".q-showOnlyMissed-btn", currQuiz).change(function () {
            ...
            if (ckd) {
              $(".q-review-index-missed", currQuiz)
                .fadeIn()
                .css("display", "inline");
              $(".q-review-index-all", currQuiz).hide();
            } else {
              $(".q-review-index-missed", currQuiz).hide();
              $(".q-review-index-all", currQuiz)
                .fadeIn()
                .css("display", "inline");
            }
            $(".q-review-index:visible", currQuiz)
              .trigger("change")
              .css("display", "inline");
          });
          setReviewNav();
        ...
        disableGameOverButtons = function () {
          if (settings.disableRestart) {
            $(".q-restart-btn", currQuiz).hide();
          }
          if (settings.disableDelete) {
            $(".q-del-btn", currQuiz).hide();
          }
        ...
        setupReview = function () {
          ...
          $(".q-review-btn", currQuiz).one("click", function () {
            ...
            if (totalQuesLen == 1) {
              $(".q-rightArrow, .q-showOnlyMissed-btn", currQuiz).attr(
                "disabled",
                true
              );
            }
        ...
        gameOver = function () {
          ...
          $(".q-statTotal", currQuiz).html(
            lang.stats.score + ": " + stats.perc() + "%"
          );
          ...
          $(
            ".q-help, .q-check-btn, .q-prob, .q-intro, .q-ans",
            currQuiz
          ).hide();
          if (settings.review) {
            setupReview();
        ...
        changeProb = function (isReview) {
          var qAS = q.ansSel;
          $(".q-counter", currQuiz).text(
            "Question " + (currIndex + 1) + " of " + totalQuesLen
          );
          ...
          if (!isReview) {
            $(".q-ansSel", currQuiz).html(
              ...
              qAS[isNaN(qAS[currIndex]) ? currIndex : qAS[currIndex]].clone(
                true
              )
            );
            setQResultDisplay(0);
        ...
        checkAns = function (isFlashCard) {
          var ans = getAns(),
            isAnsCorr = false,
            userAns = getProblemProp("userAnswer");
          ...
          if (isArray(ans)) {
            ...
            isAnsCorr =
              userAns == ans.toString() || $.inArray(userAns, ans) + 1;
          } else {
            isAnsCorr =
              userAns == ans ||
              userAns.toString().toLowerCase() == ans.toString().toLowerCase();
          }
          return isAnsCorr;
        },
        getUserAnsForDisplay = function (toUni) {
          ...
          return !getProblemProp("userAnswer")
            ? "<del>" + ans + "</del>"
        ...
        getAnsInfoForDisplay = function () {
          ...
          if (q.ansSelInfo[currIndex]) {
            return (
              ...
              lang.ans.whyAns +
              "<br/>" +
              (q.ansSelInfo[currIndex][stats.indexSelected[currIndex]] ||
                q.ansInfo[currIndex])
            );
          } else {
        ...
        displayAnsResult = function (isFlashCard) {
          var currAns = getAns(),
            isUserAnsCorr = haveCorrAnsWithinRetries(isFlashCard),
            show = "",
          ...
          show = isUserAnsCorr
            ...
            : lang.ans.corrAns +
              "<br/>" +
              getStrArrOfSeeableHTML(currAns, toUni).join("<br/>");
        ...
        releaseButtonBlock = function () {
          if (settings.showFeedback) {
            $(".q-check-btn", currQuiz).attr(
              "disabled",
              !$(".q-next-btn", currQuiz).attr("disabled")
            );
          } else {
            $(".q-next-btn", currQuiz).attr("disabled", false);
          }
        ...
        setAnsSelInfoSelect = function (quizType) {
          ...
          stats.indexSelected[currIndex] = $(groupClass, currQuiz).index(
            $(selectedClass, currQuiz)
          );
        },
        ...
        buildQuizType = {
          ...
          fillInTheBlank: function (iProp, flashVer) {
            ...
            if (flashVer) {
              d = $("<span/>")
                .addClass("clickThis userInputArea") //Enabled the checkBtn.
                .one("click", function () {
                  releaseButtonBlock();
                })
              ...
              d = $('<input type="text"/>')
                .addClass("q-quesInput focusThis userInputArea")
            ...
            var i = q.indexMax[iProp],
              qMin = q.indexMin[iProp],
              wList = wordList[q.prop[iProp]],
              numOfQues = wList.length;
          ...
          trueOrFalse: function (iProp) {
            ...
            var d = $(
              [
                '<div><input type="radio" value="1" class="true-radio trueRadio"/><label class="trueRadio">',
                lang.quiz.tfTrue,
            ...
            var currAns,
              shouldAnsBeCorr = true,
              i = q.indexMax[iProp],
              qMin = q.indexMin[iProp],
              wList = wordList[q.prop[iProp]],
              numOfQues = wList.length,
              toUni = settings.showHTML,
              tfEqual = lang.quiz.tfEqual,
              tfEnd = lang.quiz.tfEnd,
              rAS,
              cAS,
              result,
              isAnsReallyCorrect;
            ...
            while (numOfQues--) {
              ...
              if (typeof currAns != "boolean") {
                ...
                isAnsReallyCorrect =
                  !isArray(currAns) || !isArray(result)
                    ? result == currAns
                    : areArraysSame(result, currAns);
                q.ans[i] = lang.quiz[isAnsReallyCorrect ? "tfTrue" : "tfFalse"];
                q.ques[i] += tfEqual;
                q.ques[i] += getStrWithSeeableHTML(
                  isArray(result) ? result.join(",") : result,
                  toUni
                );
                q.ques[i] += tfEnd;
              } else {
              ...
              if (wList[numOfQues].ansSelInfo) {
                ...
                if (cAS && rAS) {
                  ...
                  var qAI = q.ansInfo[i];
                  q.ansSelInfo[i] = shouldAnsBeCorr
                    ? [qAI, cAS[rAS.index] || qAI]
                    : [cAS[rAS.index] || qAI, q.ansInfo[i]];
                }
              }
              i--;
            }
          ...
          multipleChoice: function (iProp, olVer) {
            ...
            $(d)
              ...
              .bind("getUserAns", function () {
                if (olVer) {
                  var elHtml = $("." + settings.activeClass, currQuiz)
                    .find(".q-ol-li-ansSel")
                    [settings.showHTML ? "text" : "html"]();
            ...
            while (numOfQues--) {
              ...
              if (hasAnsSel(iProp, numOfQues)) {
                ...
                while (len--) {
                  ...
                  x = olVer
                    ? [
                        "<li class = 'q-ol-li'><input type='radio' class='q-radioBtn'/><span class='q-ol-li-ansSel'>",
                        ansSelChoice,
                        "</span></li> ",
                      ].join("")
                ...
                while (len--) {
                  var randIndex = rNum(randAns.length),
                    ansSelIndex =
                      len == ansPos
                        ? numOfQues
                        : randAns.splice(randIndex, 1) || 0;
                  ansSelChoice = getStrWithSeeableHTML(
                    wList[ansSelIndex].ans,
                    toUni
                  );
                  x = olVer
                    ? [
        ...
        makeQuizRandom = function (randomizeQ) {
          if (randomizeQ) {
            var numArr = getNumRange(totalQuesLen),
              i = numArr.length,
              temp = { ans: [], ansSel: [], ansInfo: [], ques: [] },
              nArrI,
              qAS;
        ...
        makeQuizType = function () {
          ...
          var iProp = q.prop.length,
            currProp = "",
            p,
            allP = settings.allQuizType,
            shortProp = {
              fill: "fillInTheBlank",
              cards: "flashCard",
              tf: "trueOrFalse",
              multi: "multipleChoice",
              multiList: "multipleChoiceOl",
            };
        ...
        setToBeginningView = function () {
          ...
          $(".q-check-btn", currQuiz)
            [settings.showFeedback ? "show" : "hide"]()
            .attr("disabled", true);
        ...
        setAnimation = function (s) {
          var spd = s.fxSpeed,
            spdType = { slow: 600, fast: 200, normal: 400 };
        ...
        checkAnsSelInfo = function (ansInfo, ansSel, ansSelInfo) {
          var a = isArray(ansSel),
            b = isArray(ansSelInfo);
            ...
            for (var currKey in temp) {
              if (temp.hasOwnProperty(currKey)) {
                ...
                if (currKey == "ansSelInfo") {
                  badKey = checkAnsSelInfo(
                    temp.ansInfo || undefined,
                    temp.ansSel || undefined,
                    temp.ansSelInfo
                  );
                  break;
                }
        ...
        hasBadSyntax = function (wNames) {
          ...
          for (var prop in wNames) {
            if (wNames.hasOwnProperty(prop)) {
              if (!validKeys[prop]) {
                badKey =
                  lang.err[
                    typeof wNames === "object" ? "badQType" : "lang.err.notObj"
                  ];
              }
              if (!badKey) {
                badKey = isArray(wNames[prop])
                  ? checkQTypeKeys(wNames[prop])
                  : lang.err.notArr;
              }
              if (badKey) {
                badKey = prop + " -> " + badKey;
          ...
          if (!prop) {
            badKey = lang.err.noQType;
          }
          if (
            !badKey &&
            settings.allQuizType &&
            !validKeys[settings.allQuizType]
          ) {
