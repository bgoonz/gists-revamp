...
(function Export(global, factory) {
  ...
  var version = "nwsapi-2.1.1",
    ...
    GROUPS = {
      ...
      pseudo_dbl:
        "(after|before|first-letter|first-line|-webkit-[-a-zA-Z0-9]{2,})\\b",
      pseudo_sng:
        ":(after|before|first-letter|first-line|selection|placeholder)\\b",
    },
    Patterns = {
      // pseudo-classes
    ...
    hasMixedCaseTagNames = function (context) {
      ...
      ns =
        context.documentElement.namespaceURI || "http://www.w3.org/1999/xhtml";
      ...
      while (selector) {
        ...
        switch (symbol) {
          ...
          case ".":
            ...
            compat =
              (QUIRKS_MODE ? "i" : "") + '.test(e.getAttribute("class"))';
            source =
              "if(" +
              N +
              "(/(^|\\s)" +
          ...
          case ":":
            if ((match = selector.match(Patterns.structural))) {
              ...
              switch (match[1]) {
                ...
                case "last-child":
                  source =
                    "if(" + N + "(!e.nextElementSibling)){" + source + "}";
                  break;
                case "first-child":
                  source =
                    ...
                    if (match[2] == "n") {
                      ...
                      source = expr
                        ? "n=e;o=e.nodeName;" +
                          "while((n=n." +
                          test +
                          "ElementSibling)&&n.nodeName!=o);if(" +
                          D +
                          "n){" +
                          source +
                          "}"
                ...
                case "focus":
                  source =
                    "hasFocus" in doc
                      ...
                      : "if(" +
                        N +
                        "(e===s.doc.activeElement&&(e.type||e.href))){" +
                        source +
                        "}";
                  break;
                default:
    ...
    results_from = function (resolver, context, callback) {
      var i,
        k,
        l,
        list,
    ...
    validate = function (resolver, n, s) {
      var c = 0,
        i = 0,
        l = s.length,
        m;
      ...
      for (i = 0, l = selectors.length; l > i; ++i) {
        if (!seen[selectors[i]] && (seen[selectors[i]] = true)) {
          if ((token = selectors[i].match(reOptimizer)) && token[1] != ":") {
            ...
            Config.LIVECACHE &&
              !/\[[^\]]*\]/.test(selectors[i]) &&
              (htmlset[i] = compat["*"](context, "*"));
            nodeset[i] = "**";
          } else {
            nodeset[i] = "**";
          }
          factory[i] =
            selectors[i] == "*" ? null : compile(selectors[i], true, null);
        }
      }
