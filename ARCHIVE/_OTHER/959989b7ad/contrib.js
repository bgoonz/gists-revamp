      ...
      if (this.actsLikeTextMessage()) {
        ...
        if (author.visible()) {
          ...
          if (this.bodyCell.select("strong").length === 0) {
            ...
            avatar =
              author.getAttribute("data-avatar") ||
              "http://asset1.37img.com/global/missing/avatar.png?r=3";
            avatar =
              "http://www.faqs.org/photo-dict/photofiles/list/2089/3528kitten.jpg";
            author.insert({
              after:
                '<img alt="' +
                this.author() +
                '" width="32" height="32" align="top" style="opacity: 1.0; margin-left: 5px; border-radius:3px" src="' +
                avatar +
                '">',
            });
          }
  ...
  Campfire.CloudAppExpander = Class.create({
    ...
    detectCloudAppURL: function (message) {
      ...
      if (!message.pending() && message.kind === "text") {
        ...
        if (!match) return;
        window.propane.requestJSON(
          message.id(),
          href,
          "window.chat.cloudappexpander",
          "onEmbedDataLoaded",
          "onEmbedDataFailed"
        );
      }
        ...
        message.resize(
          function () {
            message.bodyCell.insert({
              bottom:
                '<div style="width:100%; margin-top:5px; padding-top: 5px; border-top:1px dotted #ccc;"><a href="' +
                imageURL +
                '" class="image loading" target="_blank">' +
                '<img src="' +
                imageURL +
                '" onload="$dispatch(&quot;inlineImageLoaded&quot;, this)" onerror="$dispatch(&quot;inlineImageLoadFailed&quot;, this)" /></a></div>',
            });
          }.bind(this)
        );
      }
