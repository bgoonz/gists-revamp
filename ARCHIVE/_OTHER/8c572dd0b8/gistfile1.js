...
(function IIFE(global) {
  ...
  function channel(bufSize) {
    var ch = {
      close: function __close__() {
        ...
        closeQueue(ch.take_queue, csp.CLOSED);
      },
      closed: false,
      messages: [],
      put_queue: [],
  ...
  function put(channel, value) {
    ...
    // queued put
      ...
      channel.put_queue.push(
        ...
        ASQ.iterable().then(function __then__() {
          if (!channel.closed) {
            channel.messages.push(value);
            return true;
          } else {
  ...
  function alts(actions) {
    ...
    if (open.length == 0) {
      return { value: csp.CLOSED, channel: closed };
    }
          ...
          handlers.push(
            ASQ.iterable().then(function __then__() {
              ...
              // channel already closed?
                ...
                isq.next({ value: csp.CLOSED, channel: channel });
              }
            })
          );
    ...
    return function* __go__(token) {
      ...
      if (!token.channel) {
        ...
        if (token.channel.messages.length > 0) {
          ...
          token.channel.put_queue = token.channel.messages.map(
            function __map__() {
              // make a notifiable iterable for 'put' blocking
              return ASQ.iterable().then(function __then__() {
