...
// same code as before function cachingDecorator(func) { let cache = new Map(); return function(x) { if (cache.has(x)) { return cache.get(x); } _!_ let result = func(x); // (\*\*) _/!_ cache.set(x, result); return result; }; }

alert( worker.slow(1) ); // the original method works

worker.slow = cachingDecorator(worker.slow); // now make it caching

_!_ alert( worker.slow(2) ); // Whoops! Error: Cannot read property ‘someMethod’ of undefined _/!_ \`\`\`
...
\`\``js run let worker = { slow(min, max) { alert(`Called with <span class="math inline">_min_,</span>{max}\`); return min + max; } };
...
\`\`\`js run function hash() { _!_ alert( \[\].join.call(arguments) ); // 1,2 _/!_ }
