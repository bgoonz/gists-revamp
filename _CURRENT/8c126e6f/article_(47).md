...
\`\`\`js run // 0 means 01.01.1970 UTC+0 let Jan01_1970 = new Date(0); alert( Jan01_1970 );
...
\`\`\`js run // if you are in timezone UTC-1, outputs 60 // if you are in timezone UTC+3, outputs -180 alert( new Date().getTimezoneOffset() );
...
\`\`\`js run _!_ let start = Date.now(); // milliseconds count from 1 Jan 1970 _/!_

// do the job for (let i = 0; i &lt; 100000; i++) { let doSomething = i \* i \* i; }

_!_ let end = Date.now(); // done _/!_
...
_!_ // run bench(diffSubtract) and bench(diffGetTime) each 10 times alternating for (let i = 0; i &lt; 10; i++) { time1 += bench(diffSubtract); time2 += bench(diffGetTime); } _/!_
