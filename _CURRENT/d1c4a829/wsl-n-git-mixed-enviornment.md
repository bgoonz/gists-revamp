...
`root = /` changes the mount root for windows local drives to _‘/’_ instead of the default _‘/mnt/_, so instead of C: mounted at* ’/mnt/c’* it will be mounted at _‘/c’_.
...
When doing git-checkout from a windows tool and a script file is replaced then it <s>might</s> will loose its metadata, which means loosing the execution permission. If trying to execute the script you will get a _Permission denied_, and if doing a `git diff` you will just get a difference in mode:
